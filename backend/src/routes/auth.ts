...
router.post(
  ...
  async (req: Request<unknown, unknown, LoginBody>, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const user = demoData.findDemoUserByEmail(email);
      if (!user) return res.status(401).json({ error: "Credenciales inválidas" });
      const ok = await comparePassword(password, user.password);
      if (!ok) return res.status(401).json({ error: "Credenciales inválidas" });

      const accessToken = signAccessToken({ role: user.role, providerId: user.providerId }, user.id);
      const demoRefresh = demoData.createDemoRefreshToken(user.id);
...
router.post(
  ...
  async (req: Request<unknown, unknown, RefreshBody>, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      const rt = demoData.findDemoRefreshToken(refreshToken);
      if (!rt || rt.revoked || rt.expiresAt < new Date()) return res.status(401).json({ error: "refreshToken inválido" });

      const replacement = demoData.replaceDemoRefreshToken(refreshToken);
      if (!replacement) return res.status(401).json({ error: "refreshToken inválido" });

      const user = demoData.findDemoUserById(rt.userId);
    ...
    if (refreshToken) {
      try {
        ...
        if (isPrismaUnavailable(error)) {
          demoData.revokeDemoRefreshToken(refreshToken);
        } else {
          return next(error);
        }
      }
